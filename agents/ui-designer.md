...
## Responsabilités dans le process

Tu produis dans l'ordre :
1. Frames d'écran OS de base depuis les US via `figma-generate-design` (section par section)
2. Composants Figma avec variants, tokens et 5 specs Documentation UX/UI (Anatomy · API · Color · Structure · Screen Reader)
3. Écrans secondaires et états dégradés (loading · empty · error) à partir des composants DS et du UI principal
4. Prototypes React interactifs (v1 navigation · v2 interactions complètes)
5. Tests utilisateurs sur UI finale (write-user-test-scenarios + run-user-tests)
6. Déclinaison OS secondaire (`os-decline`) après Gate 9 — mixte avec le Product Designer
7. Handoff Dev (Code Connect + package Figma)
8. Assets store

---
name: write-cadrage
description: "Pose les 13 questions de cadrage en une fois — fonctionnelles, contractuelles et planification — et structure les réponses en résumé de cadrage. Use when user says cadrage, questions initiales, démarre le cadrage, or write-cadrage."
argument-hint: "[epic-slug]"
agent: business-analyst
---

## Rôle
Poser les 10 questions de cadrage en s'appuyant sur les insights du research déjà réalisé.

> ⚠️ Ce skill est exécuté **après** le research utilisateur et les discovery interviews.
> Les personas produits sont enrichis par les données terrain, pas hypothétiques.
> Les insights de `research-plan.md`, `insights.md` et `discovery-interviews.md`
> alimentent directement les réponses aux questions Q1, Q2, Q4 et Q6.


## Prérequis
- [ ] Gate -1 validée — research stratégique + utilisateur + discovery disponibles
- [ ] Brief initial client ou backlog disponible
- [ ] Accès aux 3 fichiers research dans `specs/[project-slug]/research/`
- [ ] Retro planning du projet disponible (dates clés, jalons, deadline V1)
- [ ] Ordre de priorité des fonctionnalités défini (MoSCoW ou backlog priorisé)
- [ ] Durée et cadence des sprints de design connues (ex: sprints de 2 semaines)
...
→ Décris le flux attendu, étape par étape — en termes d'**actions et d'intentions**, pas d'interface.
→ Ne pas mentionner de composants UI (boutons, modals, listes...) ni de gestes (swipe, tap...).
→ Ex: "L'utilisateur consulte ses tâches, peut les marquer comme terminées, peut en créer de nouvelles"
→ ❌ Interdit : "L'utilisateur clique sur le bouton + pour ajouter une tâche"
→ ✅ Correct : "L'utilisateur peut ajouter une nouvelle tâche à sa liste"
...
→ Pour chaque cas d'erreur identifié, préciser les 3 éléments suivants :
  1. La **condition de déclenchement** (quand ça arrive)
  2. Ce que l'utilisateur **voit ou ressent** (impact métier)
  3. L'**action corrective** attendue du système
→ Ex: "Perte de connexion → l'utilisateur ne peut pas sauvegarder → sauvegarder en local et resynchroniser à la reconnexion"
→ Ex: "Données manquantes → l'utilisateur ne peut pas continuer → signaler le champ manquant et bloquer la progression"
...
**Questions 11-13 — Planification**

**11. Quelle est la planification des sprints de design ?**
→ Nombre de sprints prévus pour la V1.
→ Contenu de chaque sprint (quelles features / US).
→ DoD de chaque sprint (critères pour considérer le sprint terminé).
→ Ex: "Sprint 1 : onboarding + authentification (4 US). DoD : maquettes Figma validées + specs UX complètes."

**12. Quelle est l'estimation de temps par US ?**
→ Estimation UX (navigation map + flows + specs) par US ou groupe d'US.
→ Estimation UI (composants + écrans + prototype) par US ou groupe d'US.
→ Base : idéalement en jours/homme ou en points de complexité.
→ Ex: "US-001 login : UX 0.5j · UI 1j | US-002 dashboard : UX 1j · UI 2j"

**13. Quand sont planifiées les revues client ?**
→ Fréquence des revues (par sprint, bi-mensuel, à la demande...).
→ Format des revues (présentation, async Figma, prototype partagé...).
→ Qui participe côté client (décideur, PO client, utilisateur final...).
→ Ex: "Revue client à la fin de chaque sprint — vendredi. Format : prototype React partagé 48h avant."
```
...
### OS de conception de référence

> ⚠️ Cette section est **bloquante** pour le setup Figma — elle doit être définie avant toute création d'écrans.

Si les deux plateformes sont ciblées, le Product Designer choisit l'OS sur lequel concevoir en premier :

- **OS de base (conception initiale)** : [iOS / Android]
- **OS secondaire (déclinaison après Gate 9)** : [Android / iOS]
- **Justification** : [ex: cible utilisateur majoritairement iPhone / client Android-first / contrainte marché]

Le process de conception suit cet ordre :
1. Concevoir et valider tous les écrans sur l'**OS de base** (jusqu'à Gate 9)
2. Décliner vers l'**OS secondaire** après validation complète du scope
   → Agents : structure des frames, composants système, grilles
   → Product Designer : ajustements composants spécifiques et navigation

> Si une seule plateforme est ciblée, cette section est N/A.
...
> ⚠️ Cette section est **bloquante** pour l'exécution du process — elle détermine le découpage par sprint.

| Sprint | Features / US | Estimation UX | Estimation UI | DoD | Revue client |
|--------|--------------|--------------|--------------|-----|-------------|
| Sprint 1 | [US-### · US-###] | [Nj] | [Nj] | [Critères de sortie] | [Date / Format] |
| Sprint 2 | [US-### · US-###] | [Nj] | [Nj] | [Critères de sortie] | [Date / Format] |

**Retro planning**
| Jalon | Date | Description |
|-------|------|-------------|
| Démarrage | [Date] | Kick-off + research |
| Fin Sprint 1 | [Date] | [Livrables] |
| Fin Sprint N | [Date] | [Livrables] |
| Livraison V1 | [Date] | Handoff Dev complet |

**Ordre de priorité des fonctionnalités**
| Priorité | Feature | Sprint | Justification |
|----------|---------|--------|--------------|
| P0 — Must | [Feature] | Sprint 1 | [Raison] |
| P1 — Should | [Feature] | Sprint 2 | [Raison] |
| P2 — Could | [Feature] | V2 | [Raison] |
...
- Les 13 questions sont **toujours posées en une seule fois** — jamais une par une
...
5. La stack technique est-elle définie — type d'application (native, cross-platform, hybride), plateformes et versions minimum ? (oui / non — bloquant pour les étapes UI)
6. Si les deux plateformes sont ciblées, l'OS de base de conception est-il défini ? (oui / non — **bloquant**)
7. ✅ NOUVEAU [contractuel] — Le cadre des itérations client est-il défini — nombre de cycles, délai de réponse, procédure en cas de dépassement ? (oui / non — **bloquant**)
...
9. La durée et le périmètre de garantie post-livraison sont-ils définis ? (oui / non — **bloquant**)
10. Le retro planning est-il défini — jalons et deadline V1 identifiés ? (oui / non — **bloquant**)
11. Les sprints de design sont-ils définis — contenu et DoD par sprint ? (oui / non — **bloquant** : détermine le découpage du process)
12. Les estimations UX et UI sont-elles disponibles par US ? (oui / non — si non, estimer ensemble avant de valider)
13. Les dates de revues client sont-elles planifiées — par sprint ? (oui / non — **bloquant**)
14. **Critère 14** — La répartition des specs en sprint est-elle validée par le Product Designer — scope · séquence · DoD par sprint confirmés ? (oui / non — **bloquant** : le process ne démarre pas sans cette validation)

> 💡 Le skill `/suggest-sprint-plan` peut proposer une répartition automatique à partir du cadrage si tu n'as pas encore défini les sprints.

...
## Rôle
Traduire un flux fonctionnel

## Distinction flux fonctionnel vs user flow

| | Flux fonctionnel (`write-flux-fonctionnel`) | User flow (`write-user-flow`) |
|-|---------------------------------------------|-------------------------------|
| **Qui** | Business Analyst | UX Designer |
| **Quand** | Phase cadrage | Phase UX Design |
| **Quoi** | Logique métier | Séquence d'écrans et interactions |
| **Produit** | `FLUX-###.md` | `user-flow-FLUX-###.md` |

> Le user flow **ne réinvente pas** la logique — il la traduit en expérience utilisateur.
...
- [ ] `design/[feature-slug]/ux/` dossier créé
- [ ] Navigation map `SCREENS-MAP.md` disponible (`write-navigation-map` exécuté)
